...
## Implementation notes (2026-08-30, law-graph-webmcp)

- Tokens live in `src/main/resources/static/css/app.css` `:root` (`--color-*`, `--space-*`, `--radius-*`, `--dur-*`). Legacy aliases (`--text-main`, `--gold`, `--glass-border`, `--canvas-bg`) are kept because `graphView.js` inline styles read them.
- Deviations from the generated palette: background is warm ivory `#f8f6f0` (matches the 3D canvas `#f6f3ec` so the graph does not look like a hole); gold decoration `#b8912e` is border-only, gold text uses `#8a5a12` (5.6:1 on white). CJK fallbacks (`Noto Serif TC`, `Noto Sans TC`) are loaded alongside EB Garamond / Inter.
- No dark mode yet: the 3D renderer background is a JS constant (`CANVAS_BG`), so a theme switch needs `graphView.render` to read the token first.
- Verification: `npm test` (views markup) and `npm run e2e:visual` against `npm run stub` (screenshots in `docs/ui-review/<width>/`).
